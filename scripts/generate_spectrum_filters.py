...
Writes:
  - spectrum_filters.json  (1/3-oktáva + oktávy)

Fine 190–270 Hz IIR bylo nahrazeno high-res FFT (esphome/components/hlukomer_fine_fft).
"""
...
def main() -> None:
    ...
    if frag.exists():
        frag.unlink()
    print(f"wrote {path} ({len(out['bands'])} bands); removed fine IIR fragment if present")
